...
export default function ViewBoardProvider(props){
 ...
 // function to be passed via context
 ...
 function AddBoardCardItem(mainBoardId, currentBlockId, cardName) {
  ...
  function updateCardPosition(boardData){
    let newCurrentBoard
    ParsedAllBoardsItems.map(el => { 
      newCurrentBoard = boardData;
    })
    updateAllBoardsItems(JSON.stringify(ParsedAllBoardsItems))
    localStorage.setItem('AllBoards', JSON.stringify(ParsedAllBoardsItems));
 ...
 return (
      <div>
        <ViewBoardContext.Provider value={{AddListColumn:AddListColumn, currentBoard,toggleCompleteCard,updateCardPosition, UpdateCard:AddBoardCardItem}}> 
